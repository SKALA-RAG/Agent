...
def create_tools():
    ...
    tools = [
        Tool(
            ...
            description="Search the web for recent and real-time information about ICT and BioHealth startups, their funding, market trends, and competitive landscape. Use this when you need up-to-date information to supplement the 2025 industry outlook report or when looking for specific company details."
        )
    ]
...
if __name__ == "__main__":
    ...
    example_text = """
    1. **회사명**: Cresta
    2. **설립연도**: 정보 없음 (검색 결과에 구체적인 설립 연도가 언급되지 않음)
    3. **대표자**: Ping Wu (CEO)
    4. **주요 사업 분야**: AI 기반 고객 서비스 및 연락 센터 솔루션
    5. **홈페이지**: cresta.com
    6. **연락처**: 정보 없음 (검색 결과에 구체적인 연락처가 언급되지 않음)
    7. **핵심 인력 (팀 구성)**:
    * Ping Wu (CEO)
    * Tim Shi (Co-Founder, CTO)
    * Alex Cramer (CRO)
    * Russell Banzon (CMO)
    * Vahid Manie (VP of Finance)
    * Xiangru Chen (VP of Engineering)
    * Dawn McLaren (Head of People)
    * Sai Vivek (SVP of Customer Success)
    * Josh Levin (Head of Product)
    8. **주요 연혁 (설립, 투자유치, 특허 등 중요 이력)**:
    * Cresta는 AI를 통해 고부가가치 기술을 학습하도록 도와주고 비즈니스의 생산성을 극대화하는 것을 목표로 설립됨.
    * 2024년 Forbes AI 50 리스트에 선정됨 (세 번째 선정).
    * 최근 125백만 달러의 시리즈 D 자금을 유치하였으며, 총 자금 조달액은 2억 7천만 달러를 초과함.
    * Cresta Voice Virtual Agent 및 여러 혁신적인 AI 솔루션을 출시함.
    * 글로벌 확장을 위해 APAC 및 EMEA 지역으로 사업을 확장함.
    """
    startup_info = text_to_startup_info(example_text)

    result = run_market_assessment(startup_info)
    
    print("\n=== 시장성 평가 결과 ===")
    print(result["analysis_results"]["market_analysis"])